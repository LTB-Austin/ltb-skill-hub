...
- The client's picks from the Science Intelligence deck meeting: the directions they chose from the claim board, by rank and Claim ID, plus any notes. These belong in the `Client picks` block at the top of the Claim Set (see the contract); if the user gives them in chat instead, write them into that block first so the Claim Set stays the single record, then cluster around them. Their seed ideas and other feedback are secondary inputs; the picks are the backbone.
- The Category Language Map and Competitive Intel tab from the workbook, for the swap test and the voice check. If the scan is more than a few weeks old, offer to refresh it with web search before writing narratives.
...
5b. **Run the swap test.** For each cluster and each narrative, put the closest scanned competitor's name where the brand's name is. If the concept still works, it is not owned; it is a category story with our logo on it. Rewrite until the swap breaks: the direction has to rest on an `Open` claim, a differentiating element of a `Commons` claim, or a mechanism or number no competitor uses. Then check every headline and narrative against the category voice list in the Claim Set; a line that echoes a competitor's tagline or recurring phrasing is rewritten. Record the swap test result per direction on the MLR Readiness slide alongside the net impression.
...
- Swap test passed for every direction (the concept fails with a competitor's name in it), and no headline or narrative uses the category voice list. If the Claim Set says `Not scanned`, the deck says so on the MLR Readiness slide.
